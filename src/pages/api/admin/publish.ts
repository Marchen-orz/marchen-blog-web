...
function cleanSlug(value: string | undefined, fallback: string) {
...
function buildMarkdown(payload: PublishPayload) {
  ...
  if (type === "blog") {
    ...
    const slug = cleanSlug(payload.slug, `post-${timestampSlug()}`);
  ...
  const slug = cleanSlug(payload.slug, `note-${timestampSlug()}`);
